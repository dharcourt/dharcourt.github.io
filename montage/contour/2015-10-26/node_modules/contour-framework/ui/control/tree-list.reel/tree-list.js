...
exports.TreeList = Component.specialize(/** @lends TreeList.prototype */ {
        ...
        value: function () {
            if (this._controller) {
                if (this._controller.data !== this._data) {
                    ...
                    if (this._isRootExpanded || !this._isRootVisible) {
                        this._controller.expandNode(this._controller.data);
                    }
                }
            }
    ...
    */
    ...
    _addIterations: {
        value: function (node, iterationsArray, row, depth, index) {
            var length,
                children,
                nodeMetadata,
                height,
            ...
            if (node) {
                ...
                treeNode.row = row;
                if (!this._isRootVisible && (node === this._controller.data)) {
                    ...
                    if (children = this._controller.childrenFromNode(node)) {
                        ...
                        for (i = 0; i < length; i++) {
                            if (this._isVisible(row, height = this._getNodeHeight(children[i]))) {
                                row = this._addIterations(children[i], iterationsArray, row, depth + 1, i);
                            } else {
                                row += height;
                            }
                        }
    ...
    getIterations: {
        value: function () {
            ...
            if (this._controller._data && (this._visibilityRange[1] > this._visibilityRange[0])) {
                if (this._isVisible(0, this._getNodeHeight(this._controller._data))) {
                    this._addIterations(this._controller._data, iterations, 0, 0, 0);
                }
            }
            return iterations;
        }
    ...
    handleResize: {
        value: function () {
            ...
            var startRow = this._element.scrollTop / this._rowHeight,
    ...
    draw: {
        value: function () {
            var length = this.repetition._drawnIterations.length,
                iteration,
                element,
                i;
            ...
            for (i = 0; i < length; i++) {
                ...
                if (!this._isRootVisible && iteration.object.data === this._controller.data) {
